class Plant extends Growable {
  ...
  init() {
    ...
    for (let i = 1; i < this.numLeaves; i++) {
      ...
      let place = height-250-this.leafPositions[i]
    ...
    for (let i = 0; i < this.numPods; i++) {
      ...
      let pod = new Stem(this.pos.x, height-250-this.podPositions[i], this.dir, this, false)
      this.stems.push(pod)
    ...
    if (this.gobi > 0) {
      this.stalkTop = new StalkTop(this.pos.x, height-250-this.thresh, this, this.gobi)
    }
  ...
  draw() {
    // fill('blue')
    // circle(this.pos.x, height-250-this.thresh, 30) 
    // fill ('orange')
    // circle(this.pos.x, height-250-this.plantHeight, 30)
